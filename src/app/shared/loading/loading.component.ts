...
export class LoadingComponent implements AfterViewInit {
  ...
  loadParticles(): void {
    if (typeof window !== 'undefined') {
      this.renderer.listen('window', 'load', () => {
        (window as any).particlesJS('particleCanvas-Blue', {
          particles: {
            number: {
              value: 100,
              density: {
        ...
        (window as any).particlesJS('particleCanvas-White', {
          particles: {
            number: {
              value: 250,
              density: {
  ...
  startZoomOut(): void {
    if (typeof document !== 'undefined') {
      const imageElement = document.querySelector('.falling-k img') as HTMLElement;

      if (imageElement) {
        imageElement.addEventListener('animationend', () => {
          // Navigate to the login page after animation ends
